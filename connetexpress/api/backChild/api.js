...
router.post('/insertone', function(req, res, next) {
   ...
   const sqllist2 = `select id from two_class where enname = '${req.body.enname_two}'`
   let sql = `insert into one_class(id,enname,cnname,time) values('${ids}','${req.body.enname_one}','${req.body.cnname_one}','${creatTime()}')`;
   let sqls = `insert into two_class (id,parent_id,enname,cnname,article_num,time) values('${generateUUID()}','${ids}','${req.body.enname_two}','${req.body.cnname_two}',0,'${creatTime()}')`;
